-- Restore the Raw Leads cursor RPC for PostgREST/Lovable preview deployments.
-- This is intentionally separate from the original keyset migration because
-- that migration may already be recorded as applied in the remote project.
...
AS $$
DECLARE
  requested_size integer := greatest(0, least(coalesce(p_page_size, 200), 500));
BEGIN
