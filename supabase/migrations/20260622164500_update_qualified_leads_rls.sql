-- Update SELECT, INSERT, UPDATE, DELETE RLS policies on qualified_leads to support all roles (including sub_admin, maturing, etc.)
...
-- SELECT policy: Allow admin, sub_admin, cs (for their own + unassigned), and any creator to read their own leads
...
-- INSERT policy: Allow admin, sub_admin, scraping, maturing, acc_handler, facebook, and seo roles to insert
...
-- UPDATE policy: Allow admin, sub_admin, cs (assigned to them/unassigned), and creators (if status is 'new')
...
-- DELETE policy: Allow admin, sub_admin, and creators (if status is 'new')
