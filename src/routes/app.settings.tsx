      ...
      <PageBody>
        <RoleGate allow={["admin"]} current={auth.primaryRole}>
          <div className="flex gap-1 p-1 rounded-lg bg-surface border border-border inline-flex mb-5">
            <TabBtn active={tab === "general"} onClick={() => setTab("general")} icon={<SettingsIcon className="h-3.5 w-3.5" />}>General</TabBtn>
            <TabBtn active={tab === "users"} onClick={() => setTab("users")} icon={<UsersIcon className="h-3.5 w-3.5" />}>Users</TabBtn>
          </div>
  ...
  return (
    <button
      ...
      className={cn(
        ...
        active ? "bg-card text-foreground shadow-sm ring-1 ring-border-strong" : "text-muted-foreground hover:text-foreground"
      )}
    >
      {icon} {children}
    </button>
  ...
  const usersQuery = useQuery({
    ...
    queryFn: async (): Promise<UserRow[]> => {
      const [{ data: profiles, error: pErr }, { data: roles, error: rErr }] = await Promise.all([
        supabase.from("profiles").select("user_id, full_name, username, email, is_active").order("created_at", { ascending: false }),
      ...
      return (profiles ?? []).map((p) => ({
        ...
        email: p.email, is_active: p.is_active, role: roleMap.get(p.user_id) ?? null,
      }));
    },
  });
